...
export default function LearningReport({ placementResult, weeklyLessons, weeklyXp, completionPercentage = 0 }) {
...
function CertificateModal({ placementResult, onClose }) { return <div className="fixed inset-0 z-[120] bg-slate-950/70 p-4 flex items-center justify-center" role="dialog" aria-modal="true"><section className="max-w-2xl w-full bg-[#fffdf4] text-slate-900 border-[10px] border-double border-yellow-600 rounded-xl p-8 md:p-12 text-center shadow-2xl"><button onClick={onClose} aria-label="Đóng" className="float-right"><X /></button><Award size={58} className="mx-auto text-yellow-500" /><p className="text-xs tracking-[0.3em] font-black text-yellow-700 mt-4">GRAMMAR PRO · CERTIFICATE</p><h2 className="text-3xl md:text-4xl font-black mt-3">Chứng nhận hoàn thành</h2><p className="mt-5 font-bold text-slate-600">Đã hoàn thành lộ trình với kết quả đánh giá đầu vào</p><p className="text-5xl font-black text-blue-700 mt-3">{placementResult.levelLabel}</p><p className="mt-4 text-sm font-bold">Điểm đánh giá: {placementResult.score}% · Ngày cấp: {new Intl.DateTimeFormat('vi-VN', { dateStyle: 'long' }).format(new Date())}</p><button onClick={() => window.print()} className="mt-8 px-5 py-3 rounded-xl bg-slate-900 text-white font-black inline-flex items-center gap-2"><Printer size={17} /> In / Lưu PDF</button></section></div>; }
