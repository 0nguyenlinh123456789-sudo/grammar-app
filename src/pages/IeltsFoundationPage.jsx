...
import { ChibiCoach, ChibiCelebration, ChibiBadge } from '../components/common/ChibiAnimals';
import { praiseLine, greetLine, PETS, wordOfDay, petTier } from '../components/common/chibiCopy';
...
// ---------- daily buddy banner (streak + goal + word of day + feeding) ----------
function DailyBuddy({ streak = 0, dailyStats, dailyGoal = 1, lessons = 0 }) {
  const todayLessons = dailyStats?.lessons || 0;
  const goalMet = todayLessons >= dailyGoal;
  const word = useMemo(() => wordOfDay(), []);
  const tier = petTier({ lessons, streak });
  ...
  return (
    <div className={`${card} p-4 space-y-3`}>
      ...
      {/* feeding / evolution */}
      <div className="rounded-xl border-[3px] border-slate-800 dark:border-slate-600 bg-rose-50 dark:bg-rose-900/25 px-3.5 py-2.5">
        <div className="flex items-center justify-between text-xs font-black text-slate-700 dark:text-slate-200 mb-1">
          <span>🍎 Cho Mochi ăn · Cấp: {tier.label}</span>
          <span className="text-slate-400">{tier.apples} táo</span>
        </div>
        <div className="h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden"><div className="h-full bg-rose-400" style={{ width: `${Math.round(tier.progress * 100)}%` }} /></div>
        <p className="text-[11px] font-bold text-slate-400 mt-1">{tier.next ? `Học thêm ${tier.need} bài để Mochi lên cấp ${tier.next.label} ${tier.next.deco === 'crown' ? '👑' : tier.next.deco === 'cap' ? '🧢' : '🧣'}` : 'Mochi đã đạt cấp cao nhất — Bậc thầy 👑!'}</p>
      </div>
...
function Home({ onOpenChang, done, streak, dailyStats, dailyGoal }) {
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <DailyBuddy streak={streak} dailyStats={dailyStats} dailyGoal={dailyGoal} lessons={done.length} />
...
export default function IeltsFoundationPage({ completedMilestones = [], completeMilestone, streak = 0, dailyStats, dailyGoal = 1 }) {
  ...
  const handleComplete = (id) => {
    ...
    if (already) return;
    playComplete();
    const newCount = completedMilestones.length + 1; // apples app-wide (any cluster)
    const unlocked = PETS.find((p) => p.unlock.lessons === newCount);
