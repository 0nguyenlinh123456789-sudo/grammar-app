...
const ImageScanner = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto my-8 p-6 bg-white border-[4px] border-black rounded-[20px] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
      <h2 className="text-3xl font-black mb-6 text-center uppercase tracking-wider text-[#1e293b] flex items-center justify-center gap-3">
        <Camera size={32} className="text-indigo-600 animate-pulse" /> Quét Ảnh Bằng AI
      </h2>
      ...
      {!isKeySaved ? (
        <div className="mb-8 p-5 border-[3px] border-black rounded-xl bg-[#fef08a] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          <h3 className="font-bold text-lg mb-2 flex items-center gap-2">
            <Key size={20} className="text-yellow-600" /> Nhập API Key để bắt đầu:
          </h3>
        ...
        <div className="mb-6 flex justify-between items-center p-3 border-[3px] border-black rounded-xl bg-[#bbf7d0]">
          <span className="font-bold text-green-800 flex items-center gap-2">
            <CheckCircle2 size={18} className="text-green-700" /> Đã kết nối API Key thành công!
          </span>
          <button 
      ...
      {isKeySaved && (
        <div className="flex flex-col items-center gap-4">
          <label className="cursor-pointer bg-[#60a5fa] border-[3px] border-black px-8 py-3 rounded-xl font-bold text-lg hover:bg-[#3b82f6] transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-y-1 active:translate-x-1 flex items-center gap-2">
          ...
          {image && (
            ...
            >
              {loading ? (
                ...
                <span className="flex items-center justify-center gap-2"><Sparkles size={20} className="text-slate-800 fill-slate-800" /> Tạo Từ Vựng Ngay</span>
              )}
            </button>
          )}
        </div>
      ...
      {result && (
        <div className="mt-8 p-6 border-[4px] border-black rounded-[20px] bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="mb-6">
            <h4 className="font-black text-xl mb-3 bg-[#fca5a5] inline-flex items-center gap-2 px-4 py-2 border-[3px] border-black rounded-xl shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
              <Layers size={18} className="text-red-700" /> Cụm Từ Thường Gặp
            </h4>
          ...
          <div>
            <h4 className="font-black text-xl mb-4 bg-[#93c5fd] inline-flex items-center gap-2 px-4 py-2 border-[3px] border-black rounded-xl shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
              <FileText size={18} className="text-blue-700" /> Luyện Tập Mẫu Câu
            </h4>
            <div className="space-y-4">
              {result.sentences.map((sent, index) => (
                <div key={index} className="bg-[#f8fafc] p-4 border-[3px] border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <p className="text-gray-600 font-bold text-lg border-t border-dashed border-slate-200 pt-2 pl-7">
                    {sent.vi}
                  </p>
                </div>
              ))}
