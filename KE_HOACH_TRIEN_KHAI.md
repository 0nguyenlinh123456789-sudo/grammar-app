...
- **BACKLOG từ `vocabGenerated.js` (xoá 2026-08-14)**: 3 từ không có ở bất kỳ đâu trong kho — `trekking` (đi bộ đường núi), `pollution-free` (không ô nhiễm), `revegetation` (tái trồng cây). Cả ba là nội dung soạn tay thật (IPA đúng, bản dịch có dấu, ví dụ khớp), chỗ hợp lý nhất là `sustainable-tourism-vstep` (đang có 92 từ). Không gấp — xử cùng lúc với 3 từ ở trên.
