...
- **BACKLOG 6 từ: XONG 2026-08-14 — nhưng chỉ 4 từ là thật.**
  Backlog ghi 6 từ "mất theo file chết". Kiểm lại bằng phép so khớp CÓ TÁCH DẤU `/` (kho ghi cả dạng `"racket / racquet"`, `"tie / draw"`) thì **2 trong 6 vốn đã có sẵn** — phép so khớp chuỗi chính xác lúc lập backlog quá chặt nên đếm thừa:
  - ❌ `racket` — `sports-fitness-daily` đã có `racket / racquet` = "cái vợt". Thêm là trùng.
  - ❌ `draw / tie` — `sports-fitness-daily` đã có `draw` = "hòa (tỉ số)"; `sports-competition-ielts` có `tie / draw` = "trận hòa/hòa". Nghĩa đã được phủ.
  - ✅ `medal` = "huy chương" → thêm vào `sports-fitness-daily` (50 → 51 từ). Kho chỉ có `gold medal` ở topic `sports-fitness` khác.
  - ✅ `trekking`, `pollution-free`, `revegetation` → thêm vào `sustainable-tourism-vstep` (92 → 95 từ), không trùng từ cũng không trùng nghĩa trong topic.
  Nội dung lấy NGUYÊN VĂN từ lịch sử git của hai file đã xoá, không viết lại. Mô tả hai topic cập nhật theo số mới (51/95) — `tests/topic_word_counts.test.js` bắt buộc điều đó.
- **Số từ khai sai trong tiêu đề/mô tả — ĐÃ SỬA 2026-08-14** (`fe04d73`). Con số "190/244" ghi ở đây lúc đầu là ước lượng sai và **thiếu hẳn phần mô tả**; đo lại đàng hoàng: 267 chủ đề, **182/235 tiêu đề** VÀ **182/235 mô tả** khai sai; cộng dồn hứa 23.452 từ mà thực có 18.569; 73 chủ đề khai 100 trong khi có 50. Đã duyệt và làm: bỏ hẳn số khỏi **cả 235 tiêu đề** (kể cả 53 cái đang đúng) + 3 tiêu đề chặng trong `roadmapData.js`; **mô tả đặt bằng số thật**. Bảng đối chiếu đầy đủ: `BAO_CAO_SO_TU_TIEU_DE.md`. Ghim bằng `tests/topic_word_counts.test.js` + `scripts/check_topic_word_counts.mjs`.
...
5. ~~Backlog 3 từ của `sports-fitness-daily`~~ → **xong 14/08**; kiểm lại thì chỉ `medal` là thiếu thật, `racket` và `draw / tie` vốn đã có (xem mục BACKLOG ở trên).
