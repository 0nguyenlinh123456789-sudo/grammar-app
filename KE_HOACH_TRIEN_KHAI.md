...
- **#3 CÒN LẠI (đợt sau)**: 553 cặp `example`/`viExample` máy trong `vocabFinalData/MoreData/ExtendedData`; `wordFamily` + `speaking-trans` + `textbook`/`theory` tồn đọng ở `oxfordData*.js` và `oxfordAdvancedData*.js` (chưa có generator trong repo); xoá 3 unit chết của `oxfordAdvancedData.js`; nối `vocabDailyNew24.js` vào aggregate; 250 chuỗi hướng dẫn tiếng Anh + 8 mô tả chủ đề tiếng Anh (chờ duyệt bảng đối chiếu).
