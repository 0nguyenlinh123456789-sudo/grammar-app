import { AccessConfigError, jsonResponse, redisCommand, requireLearner } from '../src/server/accessCore.js';
...
export default async function handler(request, response) {
  try {
    ...
    if (request.method === 'GET') {
      const raw = await redisCommand(process.env, 'GET', key);
    ...
    const existingRaw = await redisCommand(process.env, 'GET', key);
    let existing = null;
    ...
    await redisCommand(process.env, 'SET', key, JSON.stringify({ data: body.data, updatedAt, deviceHash: session.payload.deviceHash }));
