...
| # | Tiêu chí | Đo bằng |
|---|---|---|
| N1 | Lộ trình liên tục từ **chữ cái → B2**, không đứt đoạn, không đòi người học tự đi tìm | ✅ **ĐẠT (Đợt 1)** — **710 chặng**, mọi chủ đề/unit/chủ đề ngữ pháp thuộc đúng 1 chặng: 267/267 từ vựng · 260/260 unit Oxford · 90 ngữ pháp · 12 bài A0 đứng đầu; `tests/roadmap_coverage.test.js`. ⚠️ Test N1 chỉ đếm **ba kho có từ Đợt 1** — các kho soạn sau (nghe/đọc/chép chính tả) có test độ phủ RIÊNG trong cùng file, đừng đọc N1 xanh thành "mọi kho đều có đường đi" |
| N2 | Tổng giờ lộ trình dẫn qua **≥ 450 giờ** (ước lượng thật, ghi công thức) | ✅ **ĐẠT (Đợt 1)** — **587 giờ** đến hết B2, **736 giờ** toàn lộ trình, cộng từ số bài thật của 710/710 chặng; `tests/roadmap_coverage.test.js`. Vế **"ghi công thức"** chỉ thật sự đạt từ 17/08 — xem dòng 1.5 |
...
| N6 | Có **viết được chấm** và lỗi vào sổ lỗi, ở mọi chặng ≥A2 | ~~test CI: mỗi chặng A2+ có ≥1 đề viết~~ → **đã đo lại, xem ghi chú N6 dưới bảng.** (a) **621/625** chặng A2+ có đề gắn với nội dung chặng ✅ *(đo lại 17/08; con số cũ 531/532 đứng từ khi lộ trình còn 617 chặng)* (b) ≥9 đề soạn tay có bài mẫu ✅ |
| N7 | Test đầu vào **tách được A1/A2/B1/B2/C1** | ✅ **ĐẠT (Đợt 2)** — **50 câu, đúng 10 câu mỗi bậc** A1·A2·B1·B2·C1 (mốc ≥8); bài thích ứng hỏi 12–24 câu, trình độ lấy từ nấc thang chứ không từ % đúng; `tests/placement_bank.test.js` |
| N8 | Có **bài thi cuối bậc** cho A2, B1, B2 — đủ 4 kỹ năng | ✅ **ĐẠT (Đợt 5)** — **3 đề × 4 phần** (nghe·đọc·viết·nói), **14 câu chấm được mỗi đề** (42 câu soạn tay); nhãn bậc CHỈ suy từ phần `chamDuoc: true` — phần viết/nói cần key Gemini nên không được tính vào bậc; `tests/band_exam.test.js` |
...
> **(a)** mọi chặng A2+ có một đề viết **gắn với từ vựng của chính chặng đó**, máy đóng khung nhiệm vụ quanh danh sách từ soạn tay, **KHÔNG có bài mẫu** và nói thẳng là không có — *đã đạt: **621/625** (đo lại 17/08; con số 531/532 là số đo lúc lộ trình còn 617 chặng). Bốn chặng không có đề: Oxford Unit 26 dạy hậu tố (-ful, -less, -ness) nên không thể bảo người học "dùng từ -ness trong câu", và 3 buổi chép chính tả (5 câu rời bốc lúc chạy, không có chủ đề nào để viết về); ghim bằng `tests/writing_bank.test.js`.*
...
|---|---|---|---|---|---|
| ~~1.1~~ ✅ | **XONG (Đợt 1)** — **12 bài nền tảng A0** soạn tay (`foundationData.js`): bảng chữ cái, 44 âm, quy tắc đọc, cách đọc IPA, trọng âm, nhịp câu. Cả 12 bài đều là chặng trong lộ trình, ở bậc `foundation` **đứng trước mọi bậc khác** — vế thứ hai này được ghim riêng (`roadmap_coverage.test.js`: `roadmapData[0].level === 'foundation'`), vì "nội dung có trong file" KHÔNG đồng nghĩa "có chặng dẫn tới": đúng lỗi 1.3 đã tái diễn ở bốn kho. Dùng nguyên hình dạng chủ đề ngữ pháp nên tái dùng luôn GrammarPage và cổng chấm điểm đã có. **Cụm A0 KHÔNG bị gắn nhãn "Ôn lại" với chính người đang học nó** (`roadmap_nav.test.js`). ⚠️ Dòng này bị bỏ trống chưa gạch tới 17/08 dù Đợt 1 đã ghi ✅ — cùng loại sai sổ sách với 2.2, 3.3, 3.4 | **Cụm "Mất gốc thật"**: bảng chữ cái, 44 âm, quy tắc đọc, cách đọc IPA, trọng âm | Hiện **không có gì**; người mất gốc mở app là gặp ngay IPA chưa từng được dạy | ≥12 bài, đứng trước mọi chặng khác | A | Nhỏ |
| ~~1.2~~ ✅ | **XONG (Đợt 1)** — lộ trình **44 → 710 chặng**, gấp gần 4 lần mốc "~180" đặt ra ở đây: **267/267 chủ đề từ vựng** và **90 chủ đề ngữ pháp** đều có đúng một chặng. Mốc ~180 là con số tôi đoán lúc chưa đếm kho; số thật lớn hơn vì mỗi chủ đề thành MỘT chặng thay vì gộp nhóm | **Mở lộ trình 44 → ~180 chặng**, xếp nốt **242 chủ đề từ vựng** và **59 chủ đề ngữ pháp** chưa được xếp | 91% kho từ vựng và 76% ngữ pháp đang vô hình với người đi theo lộ trình | N1 xanh | A | Trung bình |
| ~~1.3~~ ✅ | **XONG (Đợt 1)** — **260/260 unit** có chặng (elementary 60 · pre-int 100 · advanced 100), mỗi unit MỘT chặng vì chặng chỉ mang được một `targetId`. ⚠️ **Lỗi của việc này đã TÁI DIỄN ở bốn kho soạn sau** và được đo lại, sửa ở Đợt 8 — xem ghi chú dưới bảng PHẦN 1 | **Đưa 260 unit Oxford vào lộ trình** (hiện **0/260**) | Ba giáo trình đầy đủ, 12.510 câu luyện tập, không nằm trong đường đi nào | N1 xanh | A | Trung bình |
| ~~1.4~~ ✅ | **XONG (Đợt 1)** — **710/710 chặng khai `cefr`**, dùng 6 nhãn A0·A1·A2·B1·B2·C1. Bậc thêm mới `foundation` đứng đầu, **5 id bậc cũ giữ nguyên** để không ai mất tiến độ. 14 chương đầu quyển ngữ pháp "B1" xếp về A2 vì nội dung thật là A2 — đây là PHÁN ĐOÁN của bộ sinh, ghi rõ trong `build_roadmap.mjs` (`B1_A2_CUTOFF`) để sửa được | **Gắn nhãn bậc CEFR thật cho từng chặng** và chia lộ trình theo bậc A1→B2 (+C1 dự bị) | Hiện 5 tầng nhưng nhãn không khớp nội dung (file "B1" chứa cả ngữ pháp A1) | mỗi chặng có `cefr` kiểm được | A | Nhỏ |
| ~~1.5~~ ✅ | **XONG (Đợt 1) + vế "kèm công thức" bù ở 17/08 (`f483bcc`)** — **710/710 chặng khai `minutes` tính từ số bài thật**; đến hết B2 là **587 giờ**, toàn lộ trình **736 giờ** (N2 mốc ≥450 ✅). Vế **công thức** thì đến 17/08 mới có: trước đó màn hình chỉ nói "ước lượng từ số bài thật" — đúng, nhưng người học thấy 587 giờ mà không có đường tự kiểm. Nay công thức hiện ra (20 giây/mục · 4 chế độ/từ · 200 từ trong 6 phút · 2 phút/mục lý thuyết · nghe 2 lượt/bài · 3 lượt/câu chép chính tả) **kèm vế "không tính vào"**, hằng số đọc từ file máy sinh chứ không chép tay. `tests/roadmap_hours.test.js` | **Ghi giờ học ước lượng THẬT** cho từng chặng, kèm công thức | Con số ~201 giờ hiện nay là cộng các dòng mô tả viết tay | N2 xanh | A | Nhỏ |
| ~~1.6~~ ✅ | **XONG (Đợt 1)** — gợi ý chặng kế tiếp theo bậc được đề xuất + nhãn **⚠ Vượt cấp** khi đi trước quá 1 bậc, **không chặn** (bấm vào vẫn học được), cộng nhãn **Ôn lại — dưới trình độ của bạn**. Không rõ người học đang ở đâu thì KHÔNG cảnh báo ai cả. `roadmapNav.js` + `tests/roadmap_nav.test.js` (17 test) | **Khoá mềm theo thứ tự**: gợi ý chặng kế tiếp, cảnh báo khi nhảy cóc quá xa — **không chặn cứng** | 180 chặng mà không có thứ tự thì người học lạc | có gợi ý + nhãn "vượt cấp" | A | Nhỏ |
...
| ~~3.3~~ ✅ | **XONG (Đợt 4)** — số đo đã đổi sau Đợt 8, đã đo lại 17/08: **621/625 chặng A2+** (không phải 531/532 — con số cũ đứng từ khi lộ trình còn 617 chặng). **4 chặng không có đề, cả 4 đều được kê đích danh kèm lý do** trong `tests/writing_bank.test.js`: Unit 26 (dạy HẬU TỐ `-ful/-less/-ness`, không phải từ) và 3 buổi chép chính tả (5 câu rời bốc lúc chạy, không có chủ đề nào để viết về). **Ngân hàng đề viết** gắn với từ vựng của chính chặng, **9 đề soạn tay có bài mẫu**; lỗi bề mặt đổ vào sổ lỗi sẵn có. Xem ghi chú N6: cách đo cũ ("532 đề soạn tay") đã được thay bằng hai vế đo được. **Ngân hàng đề viết theo chặng** (câu → đoạn → bài 150–200 từ), chấm bằng key Gemini của khách, lỗi đổ vào sổ lỗi đã có | Hiện chỉ có **1 ô văn bản tự do** ở mục ngữ pháp | N6 xanh | **B** | Trung bình |
...
| ~~3.5~~ ✅ | **Nói: mở rộng đọc to → nói theo chủ đề** — **476/479 chặng B1+** (không phải 386/386 — đo lại 17/08 sau khi Đợt 8 mở lộ trình; 3 chặng thiếu là 3 buổi chép chính tả, kê đích danh kèm lý do) có đề, nhận xét nội dung bằng key Gemini của khách; giữ mục đọc to từng từ cho A0–A2 | Hiện chỉ so chuỗi, và bản thân việc so chuỗi không phải chấm phát âm | **476/479** chặng B1+ ✅ · 0 chặng dưới B1 ✅ *(đo lại 17/08)* · test cấm mọi lời hứa chấm phát âm | **B** | Trung bình |
...
2. **Việc 3.1 — câu hỏi thu hẹp phạm vi mà tôi đặt ra ở đây có TIỀN ĐỀ SAI, đã đo lại 17/08.**
   ...
   **✅ XONG 17/08 — 93/93 chặng, 372 câu soạn tay** (A1 47 chặng/188 câu · A2 46 chặng/184 câu). **Toàn kho nay 214 chủ đề · 856 câu** (con số 484 ở dòng 3.1 là phạm vi ≥B1 của tiêu chí N5, không phải tổng kho — hai con số khác phạm vi, cả hai đều đúng). Bánh cóc A1/A2 về 0 và đổi thành **đòi hỏi thật**: thêm một chủ đề A1/A2 đạt bốn tiêu chí mà không soạn câu hỏi là test đỏ. **53 màn hình tiếp tục hiện dòng cảnh báo cam** — 52 chặng A1/A2 không đủ điều kiện (A1 24 · A2 28) **cộng `digital-society-100` ở bậc B2**, chặng duy nhất bị loại có lý do từ đợt ≥B1. Đếm bằng `node scripts/audit_a1a2_story.mjs` cho 52 chặng kia; con số 53 là tổng thật trên màn hình. Mỗi chặng có lý do đo được, không phải một câu khái quát. Bộ lọc bốn tiêu chí là để ra DANH SÁCH VIỆC, không phải phán quyết chất lượng — chặng nào đọc rồi thấy không soạn nổi bốn câu tử tế thì loại và **ghi lý do đích danh**, như `digital-society-100`.
